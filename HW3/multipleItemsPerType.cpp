...
void selectMultipleItemsPerType(menuItemType mList[], int size, int itemQuantityList[]);
...
int main() {
  ...
  selectMultipleItemsPerType(menuList, MENU_SIZE, itemQuantityList);
...
void selectMultipleItemsPerType(menuItemType mList[], int size, int itemQuantityList[]) {
  cout << "Select multiple items of a particular type." << endl;
  while (true) {
    char ifContinue;
    cout << "Enter Y if you want to choose a type of item, otherwise, enter any other key to exit: ";
    cin >> ifContinue;
    if (ifContinue != 'Y')
      break;
    cin.ignore(numeric_limits<streamsize>::max(), '\n');
    string item;
    cout << "Enter the name of a type of item: ";
    getline(cin, item);
    int quantity;
...
void printCheck(menuItemType mList[], int size, int itemQuantityList[]) {
  ...
  for (int i=0; i < MENU_SIZE; i++) {
    if (itemQuantityList[i] > 0) {
      cout << std::left << setw(5) << itemQuantityList[i] 
      << setw(20) << mList[i].menuItem 
